...
export const JobTracking = () => {
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      ...
      // الحفاظ على مرجع الطلب النشط بعد إعادة البناء
      if (activeJob) {
  ...
  const getJobStatusName = (status) => {
    const names = { pending: 'بانتظار الفني', accepted: 'تم القبول', onway: 'في الطريق', arrived: 'قيد الصيانة', completed: 'تم الإنجاز', disputed: 'نزاع نشط' };
          ...
          {jobs.length > 0 ? (
            jobs.map(job => (
              ...
              >
                ...
                <p className="font-bold text-slate-600 dark:text-slate-300">{job.artisanName} - {job.description.substring(0, 35)}...</p>
                <span className="text-[10px] text-slate-400 mt-1 block">📅 موعد الزيارة: {job.preferredDate}</span>
      ...
      <div className="flex-1 flex flex-col gap-4">
        {activeJob ? (
          <div className="flex flex-col gap-4">
            <h3 className="text-xs font-extrabold text-brand-navy dark:text-brand-light">📍 تتبع ومحادثة الأسطى {activeJob.artisanName}</h3>
